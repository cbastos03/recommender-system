...
def sample_function(
    user_train, usernum, itemnum, batch_size, maxlen, result_queue, seed, user_labels
):
    ...
    def sample():
        ...
        labels = np.zeros([maxlen], dtype=np.float32)
        ...
        for i in reversed(user_train[user][:-1]):
            seq[idx] = i
            pos[idx] = nxt
            labels[idx] = user_labels.get(user, {}).get(nxt, 0.0)  # Ajuste para incluir labels
            if nxt != 0:
        ...
        return (user, seq, pos, neg, labels)
...
class WarpSampler(object):
    def __init__(self, User, usernum, itemnum, batch_size=64, maxlen=10, n_workers=1, user_labels=None):
            ...
            self.processors.append(
                Process(
                    ...
                    args=(
                        ...
                        user_labels  # Passando user_labels para o sampler
                    ),
                )
            )
