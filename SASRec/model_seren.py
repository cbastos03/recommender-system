...
class SASREC(tf.keras.Model):
    ...
    def train(self, dataset, sampler, **kwargs):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            if epoch % val_epoch == 0:
                ...
                print(
                    f"\nepoch: {epoch}, time: {T}, valid: {t_valid})"
                )
                print(
                    f"epoch: {epoch}, time: {T},  test: {t_test})"
                )
                t0.start()

        t_test = self.evaluate(dataset)
        print("Results:")
        for metric, values in t_test.items():
            for k, value in values.items():
                print(f"{metric}@{k}: {value}")

        return t_test


    def evaluate(self, dataset, ks=[5, 10]):
        """
        ...
        serendipity = dataset.serendipity

        NDCG = {k: 0.0 for k in ks}
        HT = {k: 0.0 for k in ks}
        HRseren = {k: 0.0 for k in ks}
        NDCGseren = {k: 0.0 for k in ks}
        valid_user = 0.0
        valid_serendipity_item = 0
        ...
        for u in tqdm(users, ncols=70, leave=False, unit="b"):
            ...
            valid_user += 1
            if serendipity[u][test[u][0]] == 1:
                valid_serendipity_item += 1
            ...
            for k in ks:
                ...
                if rank < k:
                    ...
                    if serendipity[u][test[u][0]] == 1:
                        NDCGseren[k] += 1 / np.log2(rank + 2)
                        HRseren[k] += 1


        results = {
            "HT": {k: HT[k] / valid_user for k in ks},
            "NDCG": {k: NDCG[k] / valid_user for k in ks},
            "HRseren": {k: HRseren[k] / valid_serendipity_item for k in ks},
            "NDCGseren": {k: NDCGseren[k] / valid_serendipity_item for k in ks},
        }

        return results

    def evaluate_valid(self, dataset, ks=[5, 10]):
        """
    ...
    def evaluate_serendipity(self, dataset):
        """
        Cálculo da serendipidade considerando todo o dataset (treino, validação, teste).
        """
        usernum = dataset.usernum
        itemnum = dataset.itemnum
        ...
        valid = dataset.user_valid
        test = dataset.user_test
        serendipity_ground_truth = dataset.serendipity

        user_item_serendipity_comparison = []
        ...
        for u in tqdm(users, ncols=70, leave=False, unit="user"):
            all_interactions = train[u] + valid[u] + test[u]

            if len(train[u]) < 1 or len(all_interactions) < 1:
                continue

            # user history embeddings calculation
            history_embeddings = np.array([self.item_embedding_layer(i).numpy() for i in train[u]])
            history_embeddings = history_embeddings / np.linalg.norm(history_embeddings, axis=1, keepdims=True)

            for item in all_interactions:
                item_embedding = self.item_embedding_layer(item).numpy()
                item_embedding = item_embedding / np.linalg.norm(item_embedding)

                similarities = cosine_similarity([item_embedding], history_embeddings).flatten()
                avg_similarity = np.mean(similarities)

                avg_similarity = np.clip(avg_similarity, 0, 1)

                calculated_serendipity = 1 - avg_similarity

                calculated_serendipity = np.clip(calculated_serendipity, 0, 1)

                ground_truth_serendipity = serendipity_ground_truth[u].get(item, 0)
                user_item_serendipity_comparison.append({
                    "user_id": u,
                    "item_id": item,
                    "calculated_serendipity": calculated_serendipity,
                    "ground_truth_serendipity": ground_truth_serendipity
                })

            print(f"user {u} processed. {len(all_interactions)} interactions.")

        serendipity_df = pd.DataFrame(user_item_serendipity_comparison)
        serendipity_df.to_csv("SASREC_serendipity_comparison.csv", index=False)

        print("Serendipity comparison saved")
